...
obj_pose_mssg = Pose()
url_broker = "broker.mqttdashboard.com"
broker_port = 1883
output = ""
...
def on_connect(client, userdata, flags, rc):
    rospy.loginfo("Hive connected with result: " + str(rc))
...
sub_client.connect(url_broker, broker_port)
sub_client.publish("/eyrc/aBlostcD/ros_to_iot", str(obj_pose_mssg), qos=0)
...
class SimpleActionServerTurtle:
    ...
    def __init__(self):
        ...
        self._sas = actionlib.SimpleActionServer('/action_turtle',
                                                 ...
                                                 auto_start=False)
        '''
        * '/action_turtle' - The name of the action that will be used by 
           ROS Nodes to communicate with this Simple Action Server.
        * msgTurtleAction - The Message Class that is used by ROS Actions internally for this Simple Action Server
        * execute_cb - Holds the function pointer to the function which will process 
        incoming Goals from Simple Action Clients.
        * auto_start = False - Only when self._sas.start() will be called then only 
        this Simple Action Server will start.
        '''
    ...
    # print('X: {}, Y: {}, theta: {}'.format(obj_msg_feedback.cur_x,obj_msg_feedback.cur_y,obj_msg_feedback.cur_theta))
