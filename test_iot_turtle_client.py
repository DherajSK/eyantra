...
class SimpleActionClientTurtle:
    ...
    def feedback_callback(self, feedback):
        obj_pose_mssg = Pose()
        global url_broker, broker_port
        sub_client = mqtt.Client()
        sub_client.on_connect = on_connect
        sub_client.on_message = on_message
        sub_client.connect(url_broker, broker_port)
        sub_client.publish("/eyrc/mycodenotyourcode/ros_to_iot", str(obj_pose_mssg), qos=0)
...
def main():
    ...
    sub_client.on_message = on_message
    sub_client.connect(url_broker, broker_port)
    sub_client.publish("/eyrc/mycodenotyourcode/ros_to_iot", str("asdasd"), qos=0)
    sub_client.disconnect()
